...
class DetailTvShowActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        img_favorite.setOnClickListener {
            val data = DataFilm(idfilm = data.id, name = data.name, photo = data.photo, description = data.description, date = data.date , rate = data.rate, category = "tvshow")
            ActionClick(this, idFilm, data).execute()
        }
    }
    private class InsertTask(var context: DetailTvShowActivity, var data: DataFilm) : AsyncTask<Void, Void, Boolean>() {
    ...
    private class CheckFavorite(var context: DetailTvShowActivity, var idFilm: Int) : AsyncTask<Void, Void, List<DataFilm>>() {
    ...
    private class ActionClick(var context: DetailTvShowActivity, var idFilm: Int, var data: DataFilm) : AsyncTask<Void, Void, List<DataFilm>>() {
        override fun doInBackground(vararg params: Void?): List<DataFilm> {
            return context.database!!.favoriteDao().getById(idFilm)
        }
        override fun onPostExecute(checkList : List<DataFilm>) {
    ...
    private class DeleteTask(var context: DetailTvShowActivity, var id: Long) : AsyncTask<Void, Void, Boolean>() {
        override fun doInBackground(vararg params: Void?): Boolean {
